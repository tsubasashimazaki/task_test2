        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body text-center">
                    @if (session('status'))<!-- bladeの中はif文が書ける -->
                        <div class="alert alert-success" role="alert">
                            {{ session('status') }}
                        </div>
                    @endif
                    <h2 class="create" style="color:tomato;">Edit</h2>
                    <form method="POST" action="{{ route('contact.update',['id' => $contact->id]) }}"><!-- 保存はPOST -->
                        @csrf
                        ...
                        <br>
                        性別
                        <input type="radio" name="gender" value="0" @if($contact->gender === 0 ) checked @endif >男性<!-- checkedは0だったらチェックされる -->
                        <input type="radio" name="gender" value="1" @if($contact->gender === 1 ) checked @endif >女性
                        <br>
                        年齢
                        <select name="age">
                        <option value="">選択してください</option>
                        <option value="1" @if($contact->age === 1 ) selected @endif>〜19歳</option><!-- selectedされていたらそれぞれの値 -->
